    ...
    <div class="bg-gradient-to-r from-blue-600 to-blue-700 px-8 py-6 text-white">
      <div class="flex items-center">
        ...
        <div>
          ...
          <p class="text-blue-100 mt-1">Рестораны мэдээлэл, байршил болон ажлын цагийг тохируулна уу</p>
        </div>
      </div>
    </div>
  </div>
          ...
          <ul class="space-y-1">
            {#each sections as section}
              <li>
                ...
                >
                  <svelte:component this={section.icon} size={18} class="flex-shrink-0" />
    ...
    <div class="lg:hidden w-full mb-6 -mx-4 px-4">
      <div class="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {#each sections as section}
          <button
            on:click={() => scrollToSection(section.id)}
            class="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all duration-200 flex-shrink-0 {
              activeSection === section.id
