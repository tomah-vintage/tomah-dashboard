...
<tr class="border-b border-gray-100 hover:bg-gray-50">
  <td class="p-4">
    <div class="w-12 h-12 rounded-lg overflow-hidden bg-gray-100 flex items-center justify-center">
      {#if category.image_url}
        <img src={category.image_url} alt={category.mongolian_name || category.name} class="w-full h-full object-cover" />
      {:else}
  ...
  <td class="p-4">
    <div class="text-sm text-gray-600">{formatDisplayDate(category.created_at)}</div>
  </td>
  <td class="p-4">
    <div class="text-sm text-gray-600">{formatDisplayDate(category.updated_at)}</div>
  </td>
