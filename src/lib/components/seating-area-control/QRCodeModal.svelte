...
<div class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" on:click|self={() => dispatch('close')}>
    <div class="bg-card-background p-6 rounded-lg shadow-2xl max-w-sm w-full relative">
        <button on:click={() => dispatch('close')} class="absolute top-4 right-4 text-gray-500 hover:text-gray-800">
            <X />
        </button>
        {#if table}
            ...
            <button on:click={printQR} class="mt-6 w-full bg-primary-blue text-white px-4 py-2 rounded hover:bg-blue-700">Print QR Code</button>
        {/if}
    </div>
</div>
