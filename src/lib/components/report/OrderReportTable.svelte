<script lang="ts">
  ...
  $: ordersByStatus = Object.values(OrderStatus).reduce((acc, status) => {
        ...
        <tbody class="bg-white">
          {#each statusDisplayOrder as status}
            {#if ordersByStatus[status]}
              ...
              <tr class="{config.bgClass} border-t-2 {config.borderClass.replace('border-l-', 'border-t-')}">
                <td colspan="7" class="px-6 py-4">
                  <div class="flex items-center gap-3">
                    <svelte:component this={config.icon} class="h-5 w-5 {config.iconClass}" />
                    <h4 class="font-semibold {config.textClass}">{config.label}</h4>
              ...
              {#each ordersByStatus[status] as order (order.id)}
                <tr class="hover:{config.bgClass} transition-colors border-l-4 {config.borderClass} {config.bgClass} bg-opacity-30">
                  <!-- Order Info -->
                  ...
                  <!-- Status -->
                  <td class="px-6 py-4 whitespace-nowrap">
                    <Badge variant={config.variant} class_="flex items-center gap-1 w-fit {config.badgeClass}">
