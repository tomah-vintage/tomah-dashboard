<script lang="ts">
	...
	import { Star, User, Store, MessageSquare, Calendar, X, Trash2 } from '@lucide/svelte';
...
{#if isOpen}
	...
	>
		...
		>
			<!-- Modal Header -->
			<div class="flex items-center justify-between p-6 bg-gradient-to-r from-blue-50 to-purple-50 border-b rounded-t-2xl">
			...
			{#if review}
				...
				<div class="p-6 space-y-6">
						...
						<div>
							<div class="flex items-center gap-2 mb-2">
								<div class="flex items-center">
									{#each Array(5) as _, i}
										<Star
											class="w-5 h-5 {i < review.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}"
										/>
									{/each}
								</div>
								<span class="text-lg font-semibold text-gray-900">({review.rating}/5)</span>
							</div>
							<Badge variant={getReviewTypeBadgeVariant(review.reviewType)} class="text-sm">
					...
					<div class="bg-gray-50 rounded-lg p-4">
						...
						<div class="space-y-2 text-sm">
							<div class="flex items-center gap-2">
								...
								<span class="text-gray-700">{review.user.firstName} {review.user.lastName}</span>
							</div>
					...
					<div class="bg-gray-50 rounded-lg p-4">
						...
						<div class="text-sm">
							<span class="text-gray-700 font-medium">{review.restaurant.name}</span>
						</div>
					</div>
					...
					<div class="bg-gray-50 rounded-lg p-4">
						<div class="flex items-center gap-3 mb-3">
							...
							<h3 class="font-semibold text-gray-900">Review Date</h3>
						</div>
						<div class="text-sm text-gray-700">
							<div>Created: {formatDate(review.createdAt)}</div>
							{#if review.updatedAt && review.updatedAt !== review.createdAt}
								<div class="mt-1">Updated: {formatDate(review.updatedAt)}</div>
							{/if}
						</div>
					</div>
				</div>
