<script lang="ts">
  ...
  } from "$lib/utils/restaurant-detail";
...
<div class="lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6">
  <div class="flex items-center justify-between mb-6">
    ...
    {#if restaurant.subscription}
      <div class="flex items-center gap-3">
        <span
          class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold {getSubscriptionStatusColor(
          ...
          {#if restaurant.subscription.status !== "active"}
            ...
            >
              {#if $activateSubscriptionMutation.isPending}
                <CircularLoader size="xs" color="green" />
              {:else}
    ...
    {:else}
      <span
        class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold bg-gray-100 text-gray-800"
      >
  ...
  {#if restaurant.subscription}
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <p class="text-2xl font-bold text-gray-900">
              {formatRevenue(restaurant.subscription.plan.price.toString())}
            </p>
          </div>
          <div>
            <p class="text-xs text-gray-600">Давтамж</p>
            <p class="text-sm font-medium text-gray-900">
              {restaurant.subscription.plan.interval === "monthly"
                ? "Сар бүр"
      ...
      <div class="bg-gray-50 rounded-lg p-5 border border-gray-200">
        ...
        <div class="space-y-3">
          <div>
            ...
            <p class="text-sm font-medium text-gray-900">
              {formatDate(restaurant.subscription.start_date)}
            </p>
          </div>
          <div>
          ...
          >
            ...
            {:else}
              ...
              <span class="text-sm font-semibold text-gray-700"
                >{restaurant.subscription.days_until_expiry} өдөр үлдсэн</span
              >
            {/if}
          </div>
        </div>
      ...
      <!-- Payment Summary -->
      <div
        class="md:col-span-2 bg-gray-50 rounded-lg p-5 border border-gray-200"
      >
        <h3 class="text-sm font-semibold text-gray-900 mb-4">
          Төлбөрийн хураангуй
        </h3>
        <div class="grid grid-cols-3 gap-4">
          <div class="bg-white rounded border border-gray-200 p-4 text-center">
            ...
            <p class="text-xl font-bold text-gray-900">
              {formatRevenue(
                restaurant.subscription.payment_summary.total_paid.toString(),
              )}
            </p>
          </div>
          <div class="bg-white rounded border border-gray-200 p-4 text-center">
            ...
            <p
              ...
              class:text-red-600={restaurant.subscription.payment_summary
                .outstanding_amount > 0}
              class:text-gray-900={restaurant.subscription.payment_summary
                .outstanding_amount === 0}
            >
              {formatRevenue(
                restaurant.subscription.payment_summary.outstanding_amount.toString(),
              )}
            </p>
          </div>
          <div class="bg-white rounded border border-gray-200 p-4 text-center">
      ...
      <div class="md:col-span-2">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p class="text-sm font-mono font-medium text-gray-900">
                  {restaurant.subscription.latest_invoice.invoice_number}
                </p>
              </div>
              <div>
                ...
                <p class="text-sm font-bold text-gray-900">
                  {formatRevenue(
                    restaurant.subscription.latest_invoice.amount_due.toString(),
                  )}
                </p>
              </div>
              <div>
                ...
                <p class="text-sm font-bold text-gray-900">
                  {formatRevenue(
                    restaurant.subscription.latest_invoice.amount_paid.toString(),
                  )}
                </p>
              </div>
              <div>
                ...
                <p
                  ...
                  class:text-green-600={restaurant.subscription.latest_invoice
                    .status === "paid"}
                  class:text-red-600={restaurant.subscription.latest_invoice
                    .status !== "paid"}
                >
                  {restaurant.subscription.latest_invoice.status === "paid"
                    ? "Төлөгдсөн"
                    : "Төлөгдөөгүй"}
                </p>
              </div>
        ...
        {:else}
          <div
            class="bg-gray-50 rounded-lg p-5 border border-gray-200 text-center"
          >
  ...
  {:else}
    <div class="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h3 class="text-lg font-semibold text-gray-900 mb-2">
        Захиалгын төлөвийн мэдээлэл байхгүй
      </h3>
      <p class="text-sm text-gray-500 max-w-md">
        Энэ рестораны захиалгын төлөвийн мэдээлэл одоогоор байхгүй байна.
        Системд захиалга нэмэх шаардлагатай.
      </p>
    </div>
  {/if}
</div>
