<script lang="ts">
  ...
  $: ({ data: subscriptionData, isLoading, isError, error } = $subscriptionQuery);
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <svelte:component this={getStatusIcon(subscription.status)} class="w-6 h-6 text-gray-400" />
          </div>
          <div class="space-y-3">
            <div class="flex items-center gap-2">
              <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {getSubscriptionStatusColor(subscription.status)}">
            ...
            <div class="text-sm text-gray-600">
              ...
              <div class="flex justify-between mt-1">
                ...
                <span class="font-medium" class:text-red-600={subscription.days_until_expiry <= 7} class:text-green-600={subscription.days_until_expiry > 7}>
          ...
          <div class="space-y-3">
            ...
            <div class="text-2xl font-bold text-blue-600">
              ...
              <span class="text-sm font-normal text-gray-500">
                /{subscription.plan.interval === 'monthly' ? 'сар' : subscription.plan.interval}
              </span>
            </div>
            <div class="text-sm text-gray-600">
              <div class="flex justify-between">
          ...
          <div class="space-y-3">
            <div class="text-sm text-gray-600">
              <div class="flex justify-between">
                ...
                <span class="font-medium text-green-600">{formatRevenue(subscription.payment_summary.total_paid)}</span>
              </div>
              <div class="flex justify-between mt-1">
                <span>Үлдэгдэл дүн:</span>
                <span class="font-medium" class:text-red-600={subscription.payment_summary.outstanding_amount > 0} class:text-gray-600={subscription.payment_summary.outstanding_amount === 0}>
                  {formatRevenue(subscription.payment_summary.outstanding_amount)}
                </span>
              </div>
              <div class="flex justify-between mt-1">
                <span>Хугацаа хэтэрсэн:</span>
                <span class="font-medium" class:text-red-600={subscription.payment_summary.overdue_invoices_count > 0} class:text-gray-600={subscription.payment_summary.overdue_invoices_count === 0}>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="p-4 bg-gray-50 rounded-lg">
              ...
              <p class="font-mono font-medium">{subscription.latest_invoice.invoice_number}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
              <p class="text-sm text-gray-600">Дүн</p>
              <p class="font-medium">{formatRevenue(subscription.latest_invoice.amount_due)}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
              <p class="text-sm text-gray-600">Хугацаа</p>
              <p class="font-medium">{formatDate(subscription.latest_invoice.due_date)}</p>
            </div>
            <div class="p-4 bg-gray-50 rounded-lg">
              <p class="text-sm text-gray-600">Статус</p>
              <p class="font-medium {getInvoiceStatusColor(subscription.latest_invoice.status)}">
                {subscription.latest_invoice.status === 'paid' ? 'Төлөгдсөн' : 'Төлөгдөөгүй'}
              </p>
            </div>
          </div>
        </div>
