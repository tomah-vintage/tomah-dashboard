...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const paginatedRestaurants = mockRestaurants.slice(offset, offset + page_size);

		return json({
			count: totalCount,
			next: page * page_size < totalCount ? `/api/admin/restaurants/?page=${page + 1}&page_size=${page_size}` : null,
			previous: page > 1 ? `/api/admin/restaurants/?page=${page - 1}&page_size=${page_size}` : null,
